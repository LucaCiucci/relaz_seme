...
mettere in risalto la sua natura resistiva. A questo scopo è stata esplorata
un'ampia zona della curva per alti valori di $I$ $(I\geq I_F)$, dove questa
caratteristica è maggiormente apprezzabile. Proponiamo dunque un'estensione
della legge di Shockley, tramite l'aggiunta di un termine resistivo, in grado
di descrivere la componente -ohmica- della giunzione; il che equivale a
modellare un diodo reale come un resistore in serie ad un diodo ideale.
...
sperimentali, possibilmente esagerato dalla non-linearità di risposta
...
intensità di corrente continua $I_F = 1.0$ A nominali.} si propone
un modello semplice, di resistenza parassita in serie, verificandone l'accordo
con i risultati sperimentali su un'ampia regione di punti di lavoro.
...
Onde evitare sostanziali aumenti di temperatura e le conseguenti variazioni di
resistenza nei componenti (oltre ad eventuali danni a questi stessi) si
imprimono correnti impulsate sul circuito.
...
di misurare indirettamente la corrente che circola sul diodo.
...
\begin{table}[H]% la tabella deve stare per forza sotto "seguenti:"
	...
	\caption{I valori delle resistenze poste in serie al diodo, riportate in
		valore nominale e misurate con multimetro digitale a $6\; \frac{1}{2}$
		cifre con il metodo dei 4-fili . \label{tab:res}}
    \end{center}
\end{table}
...
stabilizzato switching e da un Buck Boost Converter, alle maglie di
"carica/scarica" e di "switch" rispettivamente.
...
\begin{subfigure}{.5\textwidth}
	\centering 
 		\includegraphics[scale=0.5]{cal0.png}
	\label{fig: nofilter}
\end{subfigure}%
\begin{subfigure} {.5\textwidth}
	\centering 
		\includegraphics[scale=0.5]{cal1.png}
	\label{fig: filtered}
\end{subfigure}
\end{figure}
...
ad una tensione fissata arbitrariamente; quando il MCU misura questo valore
...
Per il procedimento si potrebbe pensare di adottare come modello la legge
...
\begin{figure}[!h]
	\centering 
		\includegraphics[scale=0.65]{100skip_linear}
...
\begin{figure}[!ht]
	\centering 
		\includegraphics[scale=0.65]{10skip_semilog}
...
derivabile entro un intervallo $[a,b]$ (con $f’(x)$ limitata e 
diversa da $0$ per ciascuna $x \in [a,b]$) ed una $v$ tale che
$f(v) = 0$. Sviluppando al prim'ordine $f$ intorno ad un generico punto $x_0$ , otterremo:
\begin{equation}
f(x) \simeq f(x_0) + f{'}(x_0) \cdot {(x-x_0)}.
\end{equation}
Nel caso in cui si voglia trovare quella $x$ tale per cui $f(x) = 0$, si otterrebbe una prima approssimazione:
\begin{equation}
x \simeq x_0   -  {\frac {f(x_0)}{f’(x_0)}}
\end{equation}
il che non ci dà molte informazioni sulla soluzione. Tuttavia è possibile iterare il procedimento, ottenendo una successione ricorsiva 
\begin{equation}
\begin{cases}
x[0] = \alpha \in [a,b] \\ 
...
\end{cases}
\end{equation}
che produce approssimazioni della radici sempre più accurate in un numero di passaggi relativamente breve.  

In pratica, per ogni iterazione tale serie valuta $f(x[i])$ e cerca la retta tangente in tale 
punto alla funzione:
\begin{equation}
...
\end{figure}
L'unica accortezza da prendere sta nello scegliere un intervallo che contenga un solo zero di $f$ (ovvero quello che si vuole determinare), che, sia questo $\bar{x}$, è anche l'unico punto fisso della successione:
\begin{equation}
 \bar{x} = \bar{x} -  \frac{f(\bar{x})}{f'(\bar{x})} \Rightarrow \bar{x}=\bar{x}.
\end{equation}
...
\begin{figure}[!ht]% questi vanno sistemati meglio perchè occupano una pagina da soli!!!
	\centering 
 		\includegraphics[scale=0.7]{Iter.png}
\end{figure}
\begin{figure}[!hbtp]
	\centering 
 		\includegraphics[scale=0.7]{vter.png}
\end{figure}
