...
In primo luogo, è stato effettuato un fit secondo le modalità descritte in 
\nameref{app: B}, con offset assunto quale parametro libero. 
...
Dai risultati ottenuti, possiamo osservare che l'ofsset risulta essere
approssimativamente nullo. Dunque è stato effettuato un fit in sua assenza.
Il grafico in cui sono stati posti i dati e la funzione di fit in quest'ultimo caso
non è stato riportato in quanto visivamente simile a \ref{fig: sck_lin}.
I parametri stimati dal fit risultano essere:
\begin{align*}
	R_{\text{diodo}} &= 46.112 \pm 0.007 \; \rm m\Omega \\
	\eta V_T &= 47.5786 \pm 0.0033 \; \rm mV \\
...
\begin{figure}[H]
	...
	\caption{Dati acquisiti e funzione di best fit \eqref{eq: model} (con offset libero) in 
...
\end{figure}
Il grafico in scala semilogaritmica riguardante la funzione di fit con offset nullo non \`e
stato riportato in quanto visivamente simile a \ref{fig: sck_log}.
...
All'interno dello \href{https://github.com/LucaCiucci/relaz_seme/blob/master/Cartella_fit/config.py}{script config.py},
\`e stata fornita la possibilit\`a di scegliere se adottare un offset quale parametro
libero. Tale opzione \`e stata introdotta per ovviare ad eventuali errori di zero dovuti agli assorbimenti di ADC in 
\verb+Teensy 3.2+.
